...
void MostrarArbol(Nodo* arbol, int contadorX, int contadorY); //Muestra todos los nodos del arbol binario
...
int AnchoPantalla(Nodo* arbol, int contador, bool izquierdo); //determina el ancho de la pantalla dependiendo del tamano del arbol
...
int main()
{
	system("mode con: cols=500"); //SE DEFINE LAS DIMENSIONES DE LA VENTANA DEL PROGRAMA
...
void Menu() {
	...
	int contadorX = 0; //posicion en x para mostrar el nodo
	int contadorY = 4; //posicion en y para mostrar el nodo
		...
		switch (decision) {
			...
			case 3: { //Mostrar arbol completo
				...
				if (arbol == NULL){ //si el arbol esta vacio
					...
					if(AnchoPantalla(arbol, 0, true) > AnchoPantalla(arbol, 0, false)){
						contadorX = 5 + 3*AnchoPantalla(arbol, 1, true);
					}
					else{
						contadorX = 5 + 3*AnchoPantalla(arbol, 1, false);
					}
					MostrarArbol(arbol, 2*contadorX, contadorY);
				}
				cout << "\n\n\n\n";
				system("pause");
				break;
...
void MostrarArbol(Nodo* arbol, int contadorX, int contadorY) { 
	if (arbol == NULL){ //Verificamos si el árbol esta vacio
		return;
	}
	else {		
		Gotoxy(contadorX, contadorY);
		cout << arbol->dato << endl; //muestra el dato del nodo
		
		MostrarArbol(arbol->derecho, contadorX + 2*contadorY, contadorY + 1); //Mostramos toda la rama derecha de la raíz 
		MostrarArbol(arbol->izquierdo, contadorX - 2*contadorY, contadorY + 2); //Mostramos toda la rama izquierda de la raíz
	}
}
...
int AnchoPantalla(Nodo* arbol, int contador, bool izquierdo){
	if (izquierdo){ //cuenta los niveles del lado izquierdo
		if(arbol->izquierdo){
			AnchoPantalla(arbol->izquierdo, contador + 1, true);
		}
		else{
			return contador;
		}	
		...
		if(arbol->derecho){ //cuenta los niveles del lado derecho
			AnchoPantalla(arbol->derecho, contador + 1, false);
		}
		else{
			return contador;
		}
